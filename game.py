...
from pieces import CustomPiece
...
PIECE_IMAGES = {
    chess.PAWN: {
        chess.WHITE: pygame.image.load("images/bbb.png"),
...
def main():
    ...
    while not gameover:
        ...
        if selected_square is not None:
            transparent_color = (*SELECTED_SQUARE_COLOR, 150)  # Add alpha value to the color tuple

            # Create a surface with per-pixel alpha
            rect_surface = pygame.Surface((SQUARE_SIZE, SQUARE_SIZE), pygame.SRCALPHA)
            pygame.draw.rect(rect_surface, transparent_color, rect_surface.get_rect())
            ...
            for move in possible_moves:
                move_square = move[1]
                alpha_surface = pygame.Surface((SQUARE_SIZE, SQUARE_SIZE), pygame.SRCALPHA)
                circle_color = (*MOVE_SQUARE_COLOR, 200)
                circle_center = (SQUARE_SIZE // 2, SQUARE_SIZE // 2)
                circle_radius = SQUARE_SIZE // 8
                pygame.draw.circle(alpha_surface, circle_color, circle_center, circle_radius)
